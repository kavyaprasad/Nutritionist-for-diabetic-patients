...
if(f.size>0):
    ...
    if (GlycemicLoad <= 10.0): #condition for low GL
        print("Food is appropriate for consumption")
    elif (GlycemicLoad <= 19.0 and sugarlevel <= 110.0): #condition for medium GL and normal blood sugar level
        print("Food is appropriate for consumption")
...
if(descision=="Y" or descision=="y"):#when the user wants a healthier substitute
    ...
    if (df.size > 0):
        ...
        if(df1.size>0):
            ...
            answer = raw_input("Is this a good substitute or are you allergic to the substitute(Y/N):")
